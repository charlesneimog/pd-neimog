...
static t_class *RenyiDivergence;
...
// ─────────────────────────────────────
static void RenyiObjBang(RenyiObj *x) {
...
// ─────────────────────────────────────
static void SetAlpha(RenyiObj *x, t_floatarg f) { x->Alpha = f; }
// ─────────────────────────────────────
static void Expo(RenyiObj *x, t_floatarg f) { x->Exp = f; }
// ─────────────────────────────────────
static void RenyiObjTick(RenyiObj *x) { outlet_float(x->Out, x->Diversity); }
// ─────────────────────────────────────
static void Normalize(RenyiObj *x, t_floatarg f) {
    if (f == 1) {
        x->Normalize = true;
    } else {
        x->Normalize = false;
...
// ─────────────────────────────────────
static void AddDsp(RenyiObj *x, t_signal **sp) {
    if (x->RealTime) {
...
// ─────────────────────────────────────
static void *RenyiObjNew(t_symbol *s, int argc, t_atom *argv) {
    RenyiObj *x = (RenyiObj *)pd_new(RenyiDivergence);
    bool isSinal = false;
    if (s == gensym("renyi~")) {
        isSinal = true;
    }
    if (argc == 2) {
        ...
        x->Clock = clock_new(x, (t_method)RenyiObjTick);
    }
...
void RenyiDivergenceSetup(void) {
    RenyiDivergence =
        class_new(gensym("renyi"), (t_newmethod)RenyiObjNew, 0, sizeof(RenyiObj), 0, A_GIMME, 0);
    class_addcreator((t_newmethod)RenyiObjNew, gensym("renyi~"), A_GIMME, 0);
    CLASS_MAINSIGNALIN(RenyiDivergence, RenyiObj, Sample);
    class_addmethod(RenyiDivergence, (t_method)AddDsp, gensym("dsp"), A_CANT, 0);
    class_addmethod(RenyiDivergence, (t_method)Normalize, gensym("norm"), A_FLOAT, 0);
    class_addmethod(RenyiDivergence, (t_method)SetAlpha, gensym("alpha"), A_FLOAT, 0);
    class_addmethod(RenyiDivergence, (t_method)Expo, gensym("exp"), A_FLOAT, 0);
    class_addbang(RenyiDivergence, RenyiObjBang);
}
